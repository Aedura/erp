...
const EventForm = ({reqType, data}: { reqType: "create" | "update"; data?: any }) => {
  ...
  return <form onSubmit={onSubmit} className='flex flex-col gap-4'>
    <h1 className='text-xl font-semibold'>{reqType === "create" ? "Create a new" : "Update"} event</h1>
    <span className='text-xs text-gray-500 font-medium'>Event Information</span>
    <div className="flex items-center justify-center gap-6 flex-wrap">
      ...
      <InputField label="Start Time" name='startTime' inputType='time' defaultVal={data?.startTime} register={register} error={errors?.startTime} />
      <InputField label="End Time" name='endTime' inputType='time' defaultVal={data?.endTime} register={register} error={errors?.endTime} />
    </div>
