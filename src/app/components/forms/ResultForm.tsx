...
const schema = z.object({
  ...
  score: z.number().positive({ message: "Please enter a valid score." }),
...
const ResultForm = ({reqType, data}: { reqType: "create" | "update"; data?: any }) => {
  ...
  return <form onSubmit={onSubmit} className='flex flex-col gap-4'>
    <h1 className='text-xl font-semibold'>{reqType === "create" ? "Create a new" : "Update"} result</h1>
    <span className='text-xs text-gray-500 font-medium'>Result Information</span>
    <div className="flex items-center justify-center gap-6 flex-wrap">
      <InputField label="Subject Name" name='subject' defaultVal={data?.subject} register={register} error={errors?.subject} />
      <InputField label="Student Name" name='student' defaultVal={data?.student} register={register} error={errors?.student} />
      <InputField label="Score" name='score' inputType='number' defaultVal={data?.score} register={register} error={errors?.score} />
